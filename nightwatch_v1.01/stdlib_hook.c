...
typedef void* (*func_memmove)(void *dest, const void *src, size_t n);
...
static func_memmove libc_memmove;
...
install_stdlibapi_hook()
{
	...
	libc_memmove = (func_memmove)dlsym(ld_handle, "memmove");
...
memmove(void *dest, const void *src, size_t n)
{
	...
	if (NightWatch_active()) {
		pre_walk((unsigned long)dest, (unsigned long)n);
		remove_sample_range((unsigned long)dest, (unsigned long)n);
		pre_walk((unsigned long)src, (unsigned long)n);
		remove_sample_range((unsigned long)src, (unsigned long)n);
	}

	disable_cache_management();
	result = libc_memmove(dest, src, n);
	enable_cache_management();
