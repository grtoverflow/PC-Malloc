...
static __thread uint64_t llc_event_cntr[NR_LLC_PERFEVENT];
                 ...
                 unsigned long addr)
{
	...
	utl_spinlock_init(&sample->lock, UTL_PROCESS_SHARED);
}
...
{
	return mprotect((void*)sample->addr, 
					PAGE_SIZE, PROT_NONE);
}
...
remove_page_sample(struct page_sample *sample)
{
	if (sample->wait_timer) {
		remove_time_event_when_pending(sample); 
		sample->wait_timer = 0;
	}
	...
	sample->chunk = NULL;
	utl_spinlock_destroy(&sample->lock);
...
#endif /* USE_ASSERT */
	if (utl_spin_trylock(&sample->lock) != 0) {
		return;
	}
...
/* stop_monit_chunk is called by the thread that frees the chunk,
 * and there should be no data race on the chunk. */
int
stop_monit_chunk(struct memory_chunk *chunk)
{
	...
	remove_time_event_when_pending(chunk); 
